...
//redux action
import { updateSpellList, updateFavouriteList } from "../../redux/modules/actions/SpellsAction";
...
const columns = [
	...
	},
	{
		...
		name: "",
		cell: row => <>
			<img height="24px" width="24px" alt={row.name} src={starOn} />
		</>
	}
...
const Home = (props) => {
	...
	const {
		...
		error,
		data: spellList,
	} = useQuery(GQL_getSpells);
	console.log(spellList)
	// if (spellList) {
	// 	updateSpellList({ spells: spellList })
	// }


	//filter items
	const filteredItems = spellList?.spells ? spellList.spells.filter(
